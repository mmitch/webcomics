#!/bin/sh
# $Id: batch.sh,v 1.8 2002-12-24 13:53:02 mitch Exp $

# $Log: batch.sh,v $
# Revision 1.8  2002-12-24 13:53:02  mitch
...
while true ; do
    ...
    FILE=pic${X}.gif
    echo -n "fetching $X: "
    wget --user-agent="${USERAGENT}" --referer=${REFBASE}/${FOLDER}/fv${X}.htm -qO${FILE} ${GETBASE}/${FOLDER}/fv${X}.gif
    if [ -s ${FILE} ]; then
	echo "OK"
	chmod -w ${FILE}
	EXITCODE=0
